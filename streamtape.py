...
# Stream
def get_download_ticket(login, key,file_id):
    url = f'https://api.streamtape.com/file/dlticket?file={file_id}&login={login}&key={key}'
    data = requests.get(url).json()
    return data


async def get_download_link(file_id, download_ticket):
...
# Upload
def upload(login, key, file_path,folder):
    sha256_hash = hashlib.sha256()
    with open(file_path, "rb") as f:
        for byte_block in iter(lambda: f.read(4096), b""):
            sha256_hash.update(byte_block)
        sha256 = sha256_hash.hexdigest()
    url = f'https://api.streamtape.com/file/ul?login={login}&key={key}&sha256={sha256}&folder={folder}'
    #       https://api.streamtape.com/file/ul?login={login}&key={key}&folder={folder}&sha256={sha256}&httponly={httponly}
    data = requests.get(url).json()
...
def add_remote_upload(login, key, file_url, folder_id, name):
    url = f'https://api.streamtape.com/remotedl/add?login={login}' \
          f'&key={key}&url={file_url}&folder={folder_id}&name={name}'
    data = requests.get(url).json()
    return data
